  ...
  def render(assigns) do
    ...
    <!-- Pixel Popover -->
      <%= if @show_pixel_popover && @commit_colors != [] do %>
        ...
        >
          <div
            class="bg-gray-800 border border-gray-600 rounded-lg shadow-xl w-[800px] h-[80vh] m-4 flex flex-col"
            phx-click="toggle_pixel_popover"
          >
            <div class="bg-gray-800 border-b border-gray-600 p-4 flex items-center justify-between flex-shrink-0">
              <h3 class="text-lg font-semibold text-gray-100">Commit Timeline - Pixel View</h3>
              <button
                phx-click="toggle_pixel_popover"
            ...
            <div class="p-6 overflow-y-auto flex-1">
              <div class="mb-4">
                ...
                <div class="bg-gray-700 p-3 rounded border border-gray-600">
                  <div
                    class="grid"
                    style={"grid-template-columns: repeat(#{calculate_grid_columns(length(@commit_colors))}, minmax(0, 1fr))"}
                  >
    ...
    <!-- Main Content -->
      <div class="flex-1 p-8 bg-gray-900">
        <div class="max-w-6xl mx-auto">
          <h1 class="text-3xl font-bold text-gray-100 mb-4 text-center">Git Commits to Colors</h1>
          <p class="text-center text-gray-400 mb-8">
            Each color box represents the first 6 characters of a commit hash from
            <%= if @commit_colors != [] do %>
              {if @commit_count == "all", do: "all", else: "the most recent #{@commit_count || "100"}"} commits
            <% else %>
          ...
          <%= if @commit_colors != [] do %>
            <div class="bg-gray-800 rounded-lg shadow-md p-6 border border-gray-700">
              <div class="flex items-center justify-between mb-4">
                ...
                <div class="text-xs text-gray-500">
                  <p>Click any pixel to view color details in the sidebar</p>
                </div>
              </div>
              <div class="grid grid-cols-20 gap-2">
                <%= for {color, index} <- Enum.with_index(@commit_colors, 1) do %>
                  <%= if rem(index, 1000) == 0 do %>
                    <div class="col-span-20 flex items-center justify-center py-2">
                  ...
                  <.tooltip text={"##{color}"} position="tooltip-top">
                    <div
                      class="w-8 h-8 rounded border border-gray-600 flex items-center justify-center text-xs font-mono cursor-pointer hover:scale-110 transition-transform"
                      style={"background-color: ##{color}"}
              ...
              <div class="flex justify-end mt-4">
                <button
                  ...
                  class="inline-flex items-center px-3 py-2 text-sm font-medium text-gray-300 bg-gray-700 border border-gray-600 rounded-md hover:bg-gray-600 hover:text-white transition-colors duration-200"
                >
          ...
          <% else %>
            <div class="text-center py-16">
              ...
              <h3 class="text-xl font-medium text-gray-200 mb-2">No Repository Loaded</h3>
    ...
    <!-- Floating Navigation Buttons -->
      <%= if @commit_colors != [] do %>
        ...
        <button
          ...
          class="fixed bottom-4 right-4 inline-flex items-center px-4 py-3 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-full shadow-lg hover:shadow-xl transition-all duration-200 z-10"
        >
